...
import { Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, Repeat, Shuffle, X, Heart } from "lucide-react"
...
export function AudioPlayer({ song, playlist, onClose, onSongChange }: AudioPlayerProps) {
  ...
  useEffect(() => {
    if (playlist?.songs && song) {
      ...
      const indices = Array.from({ length: playlist.songs.length }, (_, i) => i)
      const shuffled = [...indices].sort(() => Math.random() - 0.5)
      setShuffleOrder(shuffled)

      // Set shuffle index to current song position in shuffle order
      const shufflePos = shuffled.findIndex((i) => i === (index >= 0 ? index : 0))
  ...
  useEffect(() => {
    ...
    const updateTime = () => {
      ...
      if (!playCountUpdated && (audio.currentTime >= 30 || audio.currentTime >= audio.duration * 0.5)) {
        updatePlayCount()
  ...
  const getPreviousSongIndex = () => {
    ...
    if (isShuffle) {
      const prevShuffleIndex = shuffleIndex === 0 ? shuffleOrder.length - 1 : shuffleIndex - 1
      return shuffleOrder[prevShuffleIndex]
    } else {
      return currentSongIndex === 0 ? playlist.songs.length - 1 : currentSongIndex - 1
    }
  }
  ...
  const handlePrevious = () => {
    ...
    if (prevIndex >= 0) {
      ...
      if (isShuffle) {
        setShuffleIndex(shuffleIndex === 0 ? shuffleOrder.length - 1 : shuffleIndex - 1)
      }
          ...
          <div className="min-w-0">
            ...
            {playlist && (
              <p className="text-gray-500 text-xs truncate">
                {isShuffle ? "Shuffling" : "Playing from"} {playlist.name} • {currentSongIndex + 1} of{" "}
        ...
        <div className="flex flex-col items-center space-y-2 flex-2">
          <div className="flex items-center space-x-4">
            <Button
              ...
              className={`${isShuffle ? "text-green-500" : "text-gray-400"} ${!hasPlaylist ? "opacity-50 cursor-not-allowed" : ""}`}
              disabled={!hasPlaylist}
              title={hasPlaylist ? (isShuffle ? "Disable shuffle" : "Enable shuffle") : "Shuffle requires a playlist"}
            >
            ...
            <Button
              ...
              className={`text-gray-400 hover:text-white ${!canGoPrevious ? "opacity-50 cursor-not-allowed" : ""}`}
              disabled={!canGoPrevious}
              title={canGoPrevious ? "Previous song" : "Previous requires a playlist"}
            >
            ...
            <Button
              ...
              size="sm"
              onClick={handleNext}
              className={`text-gray-400 hover:text-white ${!canGoNext ? "opacity-50 cursor-not-allowed" : ""}`}
              disabled={!canGoNext}
        ...
        <div className="flex items-center space-x-4 flex-1 justify-end">
          ...
          <div className="flex items-center space-x-2">
            <Button
              ...
              size="sm"
              onClick={toggleLike}
              className={`${isLiked ? "text-red-500 hover:text-red-400" : "text-gray-400 hover:text-red-400"} transition-colors`}
              title={isLiked ? "Remove from liked songs" : "Add to liked songs"}
            >
              <Heart className={`w-4 h-4 ${isLiked ? "fill-current" : ""}`} />
            </Button>
            {likesCount > 0 && <span className="text-xs text-gray-400 min-w-0">{likesCount}</span>}
          </div>
